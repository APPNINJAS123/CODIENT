...
const FragmentCard=({fragment,isActiveFragment,onFragmentClick}:FragmentCardProps)=>{
        return(
            <button className={cn("flex items-start text-start gap-2 border rounded-lg bg-muted w-fit p-3 hover:bg-secondary transition-colors",isActiveFragment && "bg-primary text-primary-foreground border-primary hover:bg-primary",)} onClick={() => onFragmentClick(fragment)}>
...
const AssistantMessage=({content,fragment,createdAt,isActiveFragment,onFragmentClick,type}:AssistantMessageProps)=>{
  ...
  //new changes made to Image-added priority and suppressHydrationWarning
    return(
        <div className={cn("flex flex-col group px-2 pb-4 ",
            ...
            <div className="flex items-center gap-2 pl-2 mb-2">
                ...
                <span className="text-xs text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100">
                    {mounted ? format(createdAt, "HH:mm 'on' MMM dd, yyyy") : ""}
                </span>
